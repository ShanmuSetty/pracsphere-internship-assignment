...
import { CheckSquare, ListTodo, ClipboardList, Percent, LucideIcon } from "lucide-react";
...
type StatCardProps = {
  ...
  title: string;
  value: string | number;
  color: "teal" | "green" | "orange" | "violet";
};
...
function StatCard({ icon: Icon, title, value, color }: StatCardProps) {
  ...
  return (
    <div
      className={`p-6 rounded-2xl border border-[#3DB876]/20 backdrop-blur-xl bg-white/[0.05] flex items-center gap-4 transition-all duration-300 hover:border-[#3DB876]/50 hover:scale-[1.02] hover:shadow-2xl ${colors[color].shadow}`}
    >
